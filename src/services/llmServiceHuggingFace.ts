...
export class LLMService {
  ...
  private static readonly HUGGING_FACE_BASE = 'https://router.huggingface.co/hf-inference/v1/chat/completions';
  ...
  private static getApiKey(): string {
    ...
    if (!key) {
      throw new Error(
        'HUGGING_FACE_API_KEY not set! Add to .env: HUGGING_FACE_API_KEY=your_hugging_face_api_key'
      );
    }
    return key;
  }
  ...
  static async parseResumeToJSON(resumeText: string): Promise<ResumeData> {
    try {
      ...
      if (!this.USE_LOCAL_PARSER) {
        console.log('🤖 Using Hugging Face AI for parsing...');
        return await this.parseWithHuggingFaceAPI(resumeText);
      }
...
Extract and return ONLY the JSON object, no additional text.`;
  ...
  // ✅ NEW: Hugging Face Inference API Parser - Tries multiple models
  private static async parseWithHuggingFaceAPI(resumeText: string): Promise<ResumeData> {
    try {
      ...
      for (let i = 0; i < this.VALID_MODELS.length; i++) {
        ...
        const url = this.HUGGING_FACE_BASE;
        ...
        try {
          ...
          // Extract text from Hugging Face chat completions response format
      ...
      if (error.response?.status === 401) {
        ...
        console.error('⚠️ Model is currently loading, try again later.');
      }
