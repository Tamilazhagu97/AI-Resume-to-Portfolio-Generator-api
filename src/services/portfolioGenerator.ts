import { magazineLayout, bentoGrid, brutalistNeoBrutalism, futuristic3D, glassmorphism, neonCyberpunk, newspaperVictorian, polaroidScrapbook, swissMinimal, terminalHacker, defaultTheme } from '../templates';
import { ResumeData } from '../types';
export class PortfolioGenerator {
  static generate(data: ResumeData, templateIndex?: number): string {
    if (!data) {
      return 'Invalid data provided';
    }
    const templates = [defaultTheme, magazineLayout, bentoGrid, brutalistNeoBrutalism, futuristic3D, glassmorphism, neonCyberpunk, newspaperVictorian, polaroidScrapbook, swissMinimal, terminalHacker];
  ...
  static checkGenerate(data: ResumeData): string {
    if (!data) {
      return 'Invalid data provided';
    }
    const getTemplate = futuristic3D(data);
    return getTemplate;
  }
}